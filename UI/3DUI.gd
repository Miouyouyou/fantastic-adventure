...
export(NodePath) var np_viewport:NodePath
export(NodePath) var np_node_area:NodePath
export(NodePath) var np_node_quad:NodePath

onready var node_viewport:Viewport = get_node(np_viewport)

# Most of the code come from Godot examples projects

var node_area:Area
var node_quad:MeshInstance
var quad_mesh_size

var menu_inventory = preload("res://UI/Inventory.tscn")

var is_mouse_inside:bool = false
var is_mouse_held:bool = false
var last_mouse_pos2D:Vector2 = Vector2(0,0)
var last_mouse_pos3D:Vector3 = Vector3(0,0,0)

func add_menu(twod_ui:Node):
	clear_menu()
	node_viewport.add_child(twod_ui)

func menu_show(state:bool):
	self.visible = state
	set_process(state)
...
func menu_toggle():
	self.menu_show(!self.menu_shown())

func _hide_internal_menus():
	for child in $Viewport.get_children():
		child.visible = false

func switch_to_inventory():
	_hide_internal_menus()
	$Viewport/Inventory.visible = true

func switch_to_sketchfab():
	_hide_internal_menus()
	$Viewport/Sketchfab.visible = true


func clear_menu():
	NodeHelpers.remove_children(node_viewport)

func _ready():
	node_area = get_node(np_node_area)
	node_quad = get_node(np_node_quad)
...
func get_selected_model() -> ModelsInventory.ModelInfos:
	return $Viewport/Inventory.current_model


func _on_Sketchfab_hide():
	$Viewport/Inventory.refresh_list()
